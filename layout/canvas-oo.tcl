...
#
 oo::class create canvas-oo {
	...
	procs erase rmov amov size coords draw line poly rect oval arc ngon text csys canv

	set c $canvas
	set C(,tr) [transforms [scale 1 -1]	\
	    		[translate [expr [$c cget -width]/2.0] [expr [$c cget -height]/2.0]] {*}$Tr]
	...
	# DEBUG puts "amov: id = $id, tag id = [my tag $id], csys in = $csys, C($id,in) = $C($id,in)"
	...
	if { $csys ne "-" && $csys ne $C($id,in) } {	# Move the shape to a new csys
	    # DEBUG puts "    amov: moving shape $id to new cs $csys..."
	    set in $C($id,in)
	    # DEBUG puts "    amov: previously in cs $in"
	    ...
	    set C($id,in) $csys
	    set nin $C($id,in)
	    # DEBUG puts "    amov: shape $id now in cs $nin"
	}

	set C($id,pos) [list $type $x $y $r]
	set C($id,tr)  [transforms [rotate $r d] [scale {*}$C($id,sc)] [translate $x $y] $C($C($id,in),tr)]
	...
	if { [info exists C($id,pos)] && !$C($id,ok) } {
	    ...
	    switch $type {
	     line -
	     poly { $c coords $id [transform $tr $xy] }
	     oval {
		set ex $C($id,ex)
		set sc $C($id,sc)
		set rx $C($id,rx)
		...
		$c coords $id [transform [translate [lindex $tr 4] [lindex $tr 5]] $xy]
	     }
	     text {
		$c coords $id [transform $tr $xy]
    ...
    #
    method line { x y coords args } { my item line   {}  xx {} $x $y $coords {} {*}$args }
    method poly { x y coords args } { my item poly   {}  xx {} $x $y $coords {} {*}$args }
    method rect { args }            { my item poly   {}  rt {} {*}$args }
    ...
    method canv { x y w h id args } { my item window $id xy { -anchor } $x $y $w $h {*}$args }
	...
	set tx [transforms [rotate $r d] [scale {*}$sc] [translate $x $y] $C($in,tr)]
	...
	switch $type {
	 ...
	 oval    {
	    ...
	    set id [$c create $type [transform [translate [lindex $tx 4] [lindex $tx 5]] $xy] {*}$B]
	 }
	 window {
	    set sx [expr sqrt([lindex $tx 0]*[lindex $tx 0]+[lindex $tx 1]*[lindex $tx 1])]
	    set xy [my $expn [expr $rx*$sx] [expr $ry*$sx] {*}$A]
	    ...
	    set xy [my $expn $x $y]
	    set id [$c create $type [transform $tx {*}$xy] {*}$A]
	 }
	 default {
	    set xy [my $expn $rx $ry {*}$A]
	    set id [$c create $type [transform $tx {*}$xy] {*}$B]
